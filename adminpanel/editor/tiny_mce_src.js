			...
			if (!t.unloads) {
				...
				function fakeUnload() {
					...
					if (d.readyState == 'interactive') {
						...
						// Remove onstop listener После a while to prevent the unload function
	...
	tinymce.create('tinymce.dom.DOMUtils', {
			...
			return this.run(e, function(e) {
				...
				if (isIE) {
					...
					if (x) {
						...
						// После we use innerHTML we can fix the DOM tree
		...
		setOuterHTML : function(e, h, d) {
			...
			function setHTML(e, h, d) {
				...
				n = tp.lastChild;
				while (n) {
					t.insertПосле(n.cloneNode(true), e);
					n = n.previousSibling;
				}
		...
		insertПосле : function(node, reference_node) {
...
(function(ns) {
	...
	function Range(dom) {
		...
		extend(t, {
			...
			setStartПосле : setStartПосле,
			setEndBefore : setEndBefore,
			setEndПосле : setEndПосле,
			collapse : collapse,
		...
		function insertNode(n) {
			...
			if ((startContainer.nodeType === 3 || startContainer.nodeType === 4) && startContainer.nodeValue) {
				if (!startOffset) {
					...
					// At the end of text
					dom.insertПосле(n, startContainer);
				} else {
		...
		function _compareBoundaryPoints(containerA, offsetA, containerB, offsetB) {
			...
			// equal to the offset of B, and A is После B if its offset is greater than the
			// offset of B.
			...
			// equal to the index of the child node C and A is После B otherwise.
			c = containerB;
			...
			// the offset of B and A is После B otherwise.
			c = containerA;
			...
			// Ranges' context tree and A is После B otherwise.
		...
		function _setEndPoint(st, n, o) {
			...
			if (sc == ec) {
				// The start position of a Range is guaranteed to never be После the
				// end position. To enforce this restriction, if the start is set to
				// be at a position После the end, the Range is collapsed to that
				// position.
...
(function() {
	function Selection(selection) {
		...
		function getRange() {
			...
			function findEndPoint(start) {
				...
				if (container.nodeType == 1) {
					...
					// Move the offset if we are setting the end or the position is После an element
					if (!start || index > 0)
						offset++;
				} else {
		...
		this.addRange = function(rng) {
			...
			if (sc.nodeType == 3) {
				// Insert marker После/before startContainer
				if (skipStart)
					dom.insertПосле(marker, sc);
				else
			...
			(function() {
				...
				function selectionChange(e) {
					...
					if (e.button) {
						...
						if (pointRng) {
							// Check if pointRange is before/После selection then change the endPoint
...
(function(){
	...
	// The workaround has to do additional checks После a getElementById
...
var posProcess = function(selector, context){
	...
	// Position selectors must be done После the filter
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getContent : function(s) {
			...
			// Keep whitespace before and После
		...
		setContent : function(h, s) {
			...
			if (r.insertNode) {
				// Make caret marker since insertNode places the caret in the beginning of text После insert
				...
				function getLocation() {
					...
					function getPoint(rng, start) {
						var container = rng[start ? 'startContainer' : 'endContainer'],
							offset = rng[start ? 'startOffset' : 'endOffset'], point = [], node, childNodes, После = 0;
						...
						if (container.nodeType == 3) {
							...
							point.push(t.dom.nodeIndex(childNodes[offset], normalized) + После);
						}
		...
		getNode : function() {
			...
			if (rng.setStart) {
				// Range maybe lost После the editor is made visible again
				if (!rng)
...
/*				if (o.set)
				...
				if (!o.set) {
					...
					if (s.apply_source_formatting && s.indent_mode == 'simple') {
						// Add line breaks before and После block elements
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		renderHTML : function() {
			...
			h = '<table align="center" id="' + t.id + '" cellpadding="0" cellspacing="0" class="' + cp + ' ' + cp + 'Enabled' + (s['class'] ? (' ' + s['class']) : '') + '"><tbody><tr>';
				...
				if (o.value === undefined) {
					...
					o.onclick = function() {
						if (t.settings.onselect(o.value) !== false)
							t.select(o.value); // Must be runned После
					};
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container', {
	renderHTML : function() {
		...
		for (i=0; i<cl.length; i++) {
			...
			// Add toolbar end before list box and После the previous button
			...
			// Add toolbar start После list box and before the next button
			// This is to fix the o2k7 editor skins
			if (nx && co.ListBox) {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			if (!/TEXTAREA|INPUT/i.test(t.getElement().nodeName) && s.hidden_input && DOM.getParent(id, 'form'))
				DOM.insertПосле(DOM.create('input', {type : 'hidden', name : id}), id);
		...
		setupIframe : function() {
			...
			if (isGecko) {
				...
				// no way to move the caret behind the link. This fix adds a bogus br element После the link
		...
		setup : function() {
			...
			if (tinymce.isWebKit) {
				function insertBr(ed) {
					...
					// Place caret После BR
					rng.setStartПосле(br);
					rng.setEndПосле(br);
					selection.setRng(rng);

					// Could not place caret После BR then insert an nbsp entity and move the caret
					if (selection.getSel().focusNode == br.previousSibling) {
						selection.select(dom.insertПосле(dom.doc.createTextNode('\u00a0'), br));
						selection.collapse(TRUE);
					}

					// Create a temporary DIV После the BR and get the position as it
					// seems like getPos() returns 0 for text nodes and BR elements.
					dom.insertПосле(div, br);
...
//				return TRUE;
			...
			// Setup new before and После blocks
			bef = (sb && sb.nodeName == bn) ? sb.cloneNode(0) : ed.dom.create(bn);
			aft = (eb && eb.nodeName == bn) ? eb.cloneNode(0) : ed.dom.create(bn);

			// Remove id from После clone
			...
			// Fill empty Послеblook with current style
			if (isEmpty(aft))
...
(function(tinymce) {
	tinymce.Formatter = function(ed) {
		...
		function remove(name, vars, node) {
			...
			function removeRngStyle(rng) {
				...
				if (format.split) {
					...
					// Update range positions since they might have changed После the split operations
		...
		function expandRng(rng, format, remove) {
			...
			// This function walks up the tree if there is no siblings before/После the node
