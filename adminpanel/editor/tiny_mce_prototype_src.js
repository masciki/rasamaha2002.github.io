			...
			if (!t.unloads) {
				...
				function fakeUnload() {
					...
					if (d.readyState == 'interactive') {
						...
						// Remove onstop listener Ïîñëå a while to prevent the unload function
...
(function() {
	...
	// Patch functions Ïîñëå a class is created
	...
	tinymce.create('tinymce.dom.DOMUtils', {
			...
			return this.run(e, function(e) {
				...
				if (isIE) {
					...
					if (x) {
						...
						// Ïîñëå we use innerHTML we can fix the DOM tree
		...
		setOuterHTML : function(e, h, d) {
			...
			function setHTML(e, h, d) {
				...
				n = tp.lastChild;
				while (n) {
					t.insertÏîñëå(n.cloneNode(true), e);
					n = n.previousSibling;
				}
		...
		insertÏîñëå : function(node, reference_node) {
...
(function(ns) {
	...
	function Range(dom) {
		...
		extend(t, {
			...
			setStartÏîñëå : setStartÏîñëå,
			setEndBefore : setEndBefore,
			setEndÏîñëå : setEndÏîñëå,
			collapse : collapse,
		...
		function insertNode(n) {
			...
			if ((startContainer.nodeType === 3 || startContainer.nodeType === 4) && startContainer.nodeValue) {
				if (!startOffset) {
					...
					// At the end of text
					dom.insertÏîñëå(n, startContainer);
				} else {
		...
		function _compareBoundaryPoints(containerA, offsetA, containerB, offsetB) {
			...
			// equal to the offset of B, and A is Ïîñëå B if its offset is greater than the
			// offset of B.
			...
			// equal to the index of the child node C and A is Ïîñëå B otherwise.
			c = containerB;
			...
			// the offset of B and A is Ïîñëå B otherwise.
			c = containerA;
			...
			// Ranges' context tree and A is Ïîñëå B otherwise.
		...
		function _setEndPoint(st, n, o) {
			...
			if (sc == ec) {
				// The start position of a Range is guaranteed to never be Ïîñëå the
				// end position. To enforce this restriction, if the start is set to
				// be at a position Ïîñëå the end, the Range is collapsed to that
				// position.
...
(function() {
	function Selection(selection) {
		...
		function getRange() {
			...
			function findEndPoint(start) {
				...
				if (container.nodeType == 1) {
					...
					// Move the offset if we are setting the end or the position is Ïîñëå an element
					if (!start || index > 0)
						offset++;
				} else {
		...
		this.addRange = function(rng) {
			...
			if (sc.nodeType == 3) {
				// Insert marker Ïîñëå/before startContainer
				if (skipStart)
					dom.insertÏîñëå(marker, sc);
				else
			...
			(function() {
				...
				function selectionChange(e) {
					...
					if (e.button) {
						...
						if (pointRng) {
							// Check if pointRange is before/Ïîñëå selection then change the endPoint
