...
(function() {
	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		init : function(ed, url) {
			...
			// hidden div and placing the caret inside it and После the browser paste
			...
			function grabContent(e) {
				...
				// Styles needs to be applied После the element is added to the document since WebKit will otherwise remove all styles
				dom.setStyles(n, {
					position : 'absolute',
					left : -10000,
					top : posY,
		...
		_preProcess : function(pl, o) {
			...
			//console.log('После preprocess:' + h);
		...
		_insertPlainText : function(ed, dom, h) {
			var i, len, pos, rpos, node, breakElms, before, После,
				w = ed.getWin(),
				d = ed.getDoc(),
				sel = ed.selection,
				is = tinymce.is,
			...
			if ((typeof(h) === "string") && (h.length > 0)) {
				...
				process([
					[/<\/(?:p|h[1-6]|ul|ol|dl|table|div|blockquote|fieldset|pre|address|center)>/gi, "\n\n"],		// Block tags get a blank line После them
				...
				if ((pos = h.indexOf("</p><p>")) != -1) {
					...
					if (breakElms.length > 0) {
						...
						for (i=0, len=breakElms.length; i<len; i++) {
							...
							После += "<" + breakElms[breakElms.length-i-1].nodeName.toLowerCase() + ">";
						}

						if (pos == rpos) {
							h = before + После + h.substring(pos+7);
						}
						else {
							h = before + h.substring(pos+4, rpos+4) + После + h.substring(rpos+7);
						}
					}
				}
				...
				// Reposition the caret to the marker, which was placed immediately После the inserted content.
