...
<?php
	...
	while ($topics = mysql_fetch_array($result))
	{
		...
		if (!$topics['status']) {
		if(cfgSET('cfgApiButtonPM') == 'on') {	print "<img style=\"cursor: hand; margin-right:5px;\" onclick=\"if(confirm('Вывести автоматом API?')) top.location.href='apibutton/apibutton.php?id=".$topics['id']."&action=apibut'\" src=\"images/apipay.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"Вывести автоматом API\" />"; }
			print "<img border=\"0\" onclick=\"if(confirm('Отметить как выполнено и убрать?')) top.location.href='?a=edit&action=setstatus&id=".$topics['id']."&l=".$topics['login']."'\" src=\"images/status.png\" width=\"16\" height=\"16\" alt=\"Заявка выполнена! Убрать!\" /> ";
		}
