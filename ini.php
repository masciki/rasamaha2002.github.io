...
if ($login) {
	...
	$pebalance = $row['lr_balance'];
	
	$btbalance = $row['bt_balance'];
...
mysql_query( 'UPDATE users SET bt_balance = bt_balance + ' . $p . ' WHERE id = ' . $row['user_id'] . ' LIMIT 1' );
		mysql_query( 'INSERT INTO stat (user_id, date, plan, sum, paysys) VALUES (' . $row['user_id'] . ', ' . $row['nextdate'] . ', ' . $row['plan'] . ', ' . $p . ', ' . $row['paysys'] . ')' );

		if ($row2['days'] <= intval( $row['count'] + 1 )) {
			if ($row2['back'] == 1) {
				mysql_query( 'UPDATE users SET bt_balance = bt_balance + ' . $row['sum'] . ' WHERE id = ' . $row['user_id'] . ' LIMIT 1' );
				mysql_query( 'DELETE FROM deposits WHERE id = ' . $row['id'] . ' LIMIT 1' );
